 ...
 *
 * ENTREGA — não há WebSocket nesta infraestrutura, então o sino consulta
 * o contador a cada 30s. É barato: uma agregação sobre índice parcial, e
 * a lista só desce quando o painel é aberto. Trocar por SSE depois não
 * muda nada fora deste arquivo.
 *
 * TOAST — aparece quando o contador CRESCE em relação à última leitura,
 * e não a cada resposta. Sem essa comparação, recarregar a página faria
 * o toast reaparecer para avisos que a pessoa já viu.
 *
 * LIDAS — o contador só zera quando o usuário abre o painel, como pedido.
 * Passar o mouse ou receber uma nova não marca nada como lido.
 */
...
export default function Notificacoes() {
  ...
  const buscarContador = useCallback(async () => {
    try {
      const r = await fetch('/api/portal/notificacoes?contador=1');
      if (!r.ok) return;
      const d = await r.json();
      const n = Number(d?.naoLidas ?? 0);

      // Só avisa quando CRESCEU. Na primeira leitura da sessão apenas
      // registra a linha de base, senão o toast dispararia a cada F5.
      if (anterior.current !== null && n > anterior.current) {
        setToast(
          n - anterior.current === 1
            ? 'Você recebeu uma nova notificação'
            : `Você recebeu ${n - anterior.current} novas notificações`,
        );
      }
      anterior.current = n;
      setNaoLidas(n);
  ...
  async function abrir() {
    ...
    setAberto(true);
    try {
      const r = await fetch('/api/portal/notificacoes');
      if (!r.ok) return;
      const d = await r.json();
      setItens(d.itens ?? []);
              ...
              {itens.map((n) => {
                ...
                return (
                  <li key={n.id} className={n.lida_em ? '' : 'sino-nova'}>
                    {n.solicitacao_id ? (
                      <Link href={`/portal/${n.solicitacao_id}`} onClick={() => setAberto(false)}>
                        {corpo}
                      </Link>
                    ) : (
