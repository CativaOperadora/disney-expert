...
--   duplicada                                  -> duplicada
--
-- A coluna triagem deixa de existir. Falha de entrega de e-mail passa a
-- ser sinalizada no próprio cartão, sem tirar a solicitação do lugar.
-- =====================================================================

begin;

-- A view depende do tipo antigo e precisa sair antes.
drop view if exists fila_atendimento;

-- A restrição cita o valor 'perdido' e também precisa sair antes.
...
alter table solicitacoes
  ...
  alter column status type status_solicitacao_novo
    ...
    )::status_solicitacao_novo,
  alter column status set default 'nova_solicitacao';
...
-- Para quando a solicitação sai de 'nova_solicitacao'. É a métrica que
-- mede a Cativa e não depende da agência responder.
-- ---------------------------------------------------------------------
alter table solicitacoes
...
-- Registros antigos que já não estão em nova_solicitacao ganham o marco
-- a partir do primeiro evento de mudança de status, ou da criação.
...
-- ---------------------------------------------------------------------
-- Prazo por tier da agência, em horas corridas
-- ---------------------------------------------------------------------
...
comment on column agencias.sla_horas is
  'Prazo de primeiro atendimento em horas corridas. Select 24, demais 48. Ajustado automaticamente pelo gatilho ao mudar o tier.';

-- O toggle "Agência SELECT" muda o tier, e o prazo acompanha sozinho.
